...
$.each($paragraphs, function(i, p) {
    if (isOverflowing(p)) {
        $($allgroups[i])
            .find('.group-card-read-more-hidden')
            .toggleClass('group-card-read-more-hidden');
    }
});
...
$($allgroups.find('.group-card-read-more')).click(function() {
    ...
    $(this)
        .siblings('p')
        .toggleClass('group-card-description');
    $(this)
        .siblings('p')
        .toggleClass('group-card-description-expanded');

    $(this)
        .parent()
        .toggleClass('group-card-info');
    $(this)
        .parent()
        .toggleClass('group-card-info-expanded');
});
